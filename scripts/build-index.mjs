...
// ajuste os diretórios de origem conforme sua árvore
const DATA_DIRS = ["data/affix/raw", "data/alter/raw"];
...
function pdftotext(file) {
  return new Promise((res, rej) => {
    execFile(
      "pdftotext",
      ["-layout", "-enc", "UTF-8", "-q", file, "-"],
      { maxBuffer: 80 * 1024 * 1024 },
      (err, stdout) => (err ? rej(err) : res(stdout.toString("utf8")))
    );
  });
}
...
async function run() {
  ...
  const patterns = DATA_DIRS.map((d) => path.join(ROOT, d, "**/*.pdf"));
  ...
  for (const abs of files) {
    ...
    const base = path.parse(abs).name; // robusto p/ .PDF/.pdf
    const id = `${base}-${sha(rel)}`;
